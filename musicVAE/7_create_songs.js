 ...
 */
function receivePhrase(phrase, chord, i)
{
...
function receiveSong(song, i)
{
  ...
  const midi = mm.sequenceProtoToMidi(progSeq);
    console.log(i);
  writeMidiToDisk(midi, `songs/song_${i}.mid`);
}
...
prom = new Promise((j) => model
  ...
  // })
  .then(() => {  
    fs.promises.readFile("sample_1.txt", 'ascii')
    .then((filedata) => {
 ...
 sample.pop()
    ...
    for (let i=0;i<chords.length;i++)
    {
      ...
      .then((phrase) => 
      {
        ...
        receivePhrase(phrase[0], chords[i], j);
      }
      );
    }
...
`
async function run_model(j){
    console.log("running model");
    song = []
    await model.initialize()
    filedata = await fs.promises.readFile("sample_1.txt", 'ascii')
    let sample = [];
    filedata.split(',').map((item) => {sample.push(+(item) + (0.25 * Math.random()))})
    sample.pop()
    console.log(sample[1])
    ...
    for (let i=0;i<chords.length;i++)
    {
        phrase = await model.decode(z, undefined, {chordProgression:[chords[i]]}, STEPS_PER_QTR)
        console.log("Processing model output "+phrase.length);
        receivePhrase(phrase[0], chords[i], j);
    }
    console.log("End of loop");
